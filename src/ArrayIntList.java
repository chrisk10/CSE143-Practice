...
public class ArrayIntList {
    ...
    /* My practice problems down here
     * 4/26/18 Practicing old midterm tests
     */
    ...
    public ArrayIntList fromCounts(ArrayIntList other) {
    		...
    		int index = 0;
    		for (int i = 1; i < size; i = i + 2) {
    			for (int j = 0; j < other.elementData[i - 1]; j++) {
    				ret.elementData[index] = other.elementData[i];
    				index++;
    			}
    		}
    		ret.size = index;
    ...
    public boolean isConsecutive() {
    		for (int i = 0; i < size - 1; i++) {
    			if (elementData[i] + 1 != elementData[i + 1]) {
    				return false;
    			}
    		}
    		return true;
